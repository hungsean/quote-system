...
- **FR-005**: 系統 MUST 允許用戶新增商品，並存儲商品名稱、描述和類別
- **FR-006**: 系統 MUST 驗證新增商品時的必填字段（名稱、描述、類別）
...
- **Product**: 表示用戶新增的商品。必要屬性包括名稱、描述、類別、所有者 ID、創建日期等。一個商品可以關聯到多個估價。
- **Quote**: 表示對商品的估價。屬性包括商品 ID、估價結果、估價日期、所有者 ID 等。一個估價隸屬於一個商品和一個用戶。
